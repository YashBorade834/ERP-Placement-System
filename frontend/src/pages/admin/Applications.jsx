...
export default function Applications() {
  ...
  const filteredApplications = applications.filter(app => {
    ...
    const matchesStatus =
      filterStatus === "ALL" ||
      normalizeStatus(app.application_status) === filterStatus;
  ...
  const stats = {
    ...
    applied: applications.filter(
      a => normalizeStatus(a.application_status) === "APPLIED"
    ).length,

    shortlisted: applications.filter(
      a => normalizeStatus(a.application_status) === "SHORTLISTED"
    ).length,

    selected: applications.filter(
      a => normalizeStatus(a.application_status) === "SELECTED"
    ).length,

    rejected: applications.filter(
      a => normalizeStatus(a.application_status) === "REJECTED"
    ).length,
  };
                ...
                <tbody>
                  {filteredApplications.map(app => (
                    <tr key={app.id}>
                      ...
                      <td>
                        <span className={`erp-pill ${statusPillClass(normalizeStatus(app.application_status))}`}>
                          {statusEmoji(normalizeStatus(app.application_status))}
                          {" "}
                  ...
                  {roundStatuses.map((round, idx) => (
                    <div key={round.round_id} className="erp-mb-4" style={{
                      ...
                      <div style={{
                        ...
                        <span className={`erp-pill ${statusPillClass(normalizeStatus(round.status))}`} style={{ fontSize: "10px" }}>
                          {statusEmoji(normalizeStatus(round.status))} {round.status}
                        </span>
                      </div>
